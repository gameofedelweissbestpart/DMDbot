...
class ConfirmClearView(discord.ui.View): # ใช้ View ตามปกติของ Discord.py
    ...
    async def confirm(self, it: discord.Interaction, b: discord.ui.Button):
        # [1] แจ้งข้อความลับเริ่มต้น (Ephemeral)
        await it.response.defer(ephemeral=True)
        
        # [2] ส่วนการส่งไฟล์ Backup ให้ผู้กดคนเดียว
        try:
            f_send = []
            ...
            if f_send:
                await it.user.send(
                    f"📦 **Backup สำเร็จ:** นี่คือไฟล์ข้อมูลก่อนการล้างระบบโดยคุณ {it.user.display_name}", 
                    files=f_send
                )
        except discord.Forbidden:
            # แจ้งเตือนหากผู้กดปิด DM ไว้
            await it.followup.send("❌ ไม่สามารถส่งไฟล์ได้เนื่องจากท่านปิด DM โปรดเปิดแล้วลองใหม่ครับ", ephemeral=True)
            return

        # [3] ลอจิกการกรองข้อมูล (ลบรายการที่เก่ากว่า 30 วัน)
        d = load_json(DB_LEAVE, [])
        now = get_thai_time().date()
        ...
        # [4] บันทึกข้อมูลและอัปเดตบอร์ด[cite: 4]
        save_json(DB_LEAVE, filtered_data)
        await update_summary_board()
        
        # [5] แสดงข้อความตอบกลับ และสั่งให้หายไปเองภายใน 3 วินาที[cite: 4]
        ...
        # ส่ง Log เข้าห้องที่ตั้งค่าไว้[cite: 4]
        cfg = load_json(CONFIG_PATH, {})
        log_ch_id = cfg.get("log_ch")
        if log_ch_id:
            log_ch = bot.get_channel(int(log_ch_id))
        ...
        # รอ 3 วินาทีแล้วลบข้อความลับทิ้ง[cite: 4]
        await asyncio.sleep(3)
        try:
            await it.delete_original_response()
        except:
