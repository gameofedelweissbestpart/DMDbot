...
class MemberPaymentView(discord.ui.View):
    ...
    @discord.ui.button(label="💸 ชำระค่าปรับ", style=discord.ButtonStyle.success, custom_id="pay_btn")
    async def pay(self, it, b):
        f_data = load_fines()
        amt = f_data["unpaid_fines"].get(str(it.user.id), 0)
        if amt <= 0: return await it.response.send_message("✅ คุณไม่มียอดค้างชำระ", ephemeral=True, delete_after=5)
        await it.response.send_modal(PaymentModal(amt, it.user.id))

    @discord.ui.button(label="🔍 ตรวจสอบประวัติของฉัน", style=discord.ButtonStyle.secondary, custom_id="chk_btn")
    async def check_me(self, it, b):
        f_data = load_fines()
        amt = f_data["unpaid_fines"].get(str(it.user.id), 0)
        msg = f"**📊 ข้อมูลค่าปรับของคุณ: {it.user.display_name}**\n▫️ ยอดค้างชำระปัจจุบัน: `{amt:,}` WD\n{LONG_SEP}"
        await it.response.send_message(msg, ephemeral=True, delete_after=5)
...
class AttendanceView(discord.ui.View):
    ...
    async def close_menu(self, it: discord.Interaction, b):
        ...
        # 1. ส่ง Log ไปที่ห้อง "📌 Log การปรับเงิน" (ชิดซ้ายสุดขอบ)
        await send_fine_log(it.guild, "บันทึกออกบิลค่าปรับ", 
                            f"▫️ ยอดรวมบิลใหม่: `{total_fine_today:,}` WD\n"
                            f"**📄 รายละเอียด:**\n{log_details}", it.user.display_name)
        
        # 2. แจ้งสถานะให้แอดมินทราบ (เห็นคนเดียว)
        await it.response.send_message("✅ บันทึกยอดและลง Log เรียบร้อย!", ephemeral=True)

    # --- เพิ่มปุ่มปิดเมนู ต่อท้ายตรงนี้เลย ---
    @discord.ui.button(label="🔴 ปิดเมนู", style=discord.ButtonStyle.danger, row=4)
    async def close_menu(self, it: discord.Interaction, b):
        await it.message.delete()
